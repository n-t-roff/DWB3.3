 ...
 */
	...
	ERROR(FATAL, 1, (stderr, "can't read %s", prologue));
 ...
 */
	...
	switch ( ch ) {
	    ...
	    case 'J':
			...
			ERROR(FATAL, 1,
			    (stderr, "can't open accounting file %s", optarg));
		    break;
	    ...
	    case 'C':			/* copy file to output */
		    if ( cat(optarg) == FALSE )
			ERROR(FATAL, 1, (stderr, "can't read %s", optarg));
		    break;
	    ...
	    default:
		    ERROR(FATAL, 1, (stderr, "missing case for option %c", ch));
		    break;
	}   /* End switch */
    }	/* End while */
 ...
 */
	...
	if ( cat(formfile) == FALSE )
	    ERROR(FATAL, 1, (stderr, "can't read %s", formfile));
 ...
 */
	...
	while ( argc > 0 ) {
		...
		ERROR(FATAL, 1, (stderr, "can't open %s", *argv));
	    conv(fp);
	    if ( fp != stdin )
		fclose(fp);
	    argc--;
 ...
 */
	...
	switch (c) {
	    ...
	    case 'D':			/* drawing functions */
			...
			default:
			    ERROR(FATAL, 1, (stderr, "unknown drawing function %c", c));
			    break;
		    }	/* End switch */
		    break;
	    ...
	    default:
		    ERROR(FATAL, 1, (stderr, "unknown input character %o %c", c, c));
		    done();
	}   /* End switch */
    }	/* End while */
 ...
 */
	...
	if ( name[0] == '\0' ) {
	    ...
	    if ( mountfont(path, m) == -1 ) {
		sprintf(path, "%s/dev%s/%s", fontdir, realdev, f);
		ERROR(FATAL, 1, (stderr, "can't load %s at %d", path, m));
	    }	/* End if */
	} else ERROR(FATAL, 1, (stderr, "can't load %s at %d", path, m));
    }	/* End if */
 ...
 */
	...
	if ( strcmp(dwb_devname, realdev) ) {
	    sprintf(path, "%s/dev%s/DESC", fontdir, dwb_devname);
	    if ( checkdesc(path) )
		realdev = dwb_devname;
	}   /* End if */
	...
	if ( getdesc(path) == -1 )
	    ERROR(FATAL, 1, (stderr, "can't open %s", path));
	nfonts = 0;
	gotspecial = FALSE;
 ...
 */
    ...
    if ( seenpage == TRUE ) {
	if ( n < 0 || n > nfonts )
	    ERROR(FATAL, 1, (stderr, "illegal font position %d", n));
 ...
 */
    ...
    if ( m < 0 || m > MAXFONTS )
	ERROR(FATAL, 1, (stderr, "illegal font %d", m));
    font = m;
 ...
 */
	...
	lastw = widthfac * (((int)dwb_mount[k]->wp[i].wid * size + unitwidth/2) / unitwidth);
	oput(code);
    }	/* End if */
