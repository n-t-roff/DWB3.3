...
int	pstab[MAXPS];			/* point size table */
int	nsizes;				/* number in DESC */

int	sync[100];			/* to troff position after printing */
int	synccount = 0;			/* next sync character */
 ...
 */
	...
	if ( strcmp(buf, "res") == 0 )
	    ...
	    nsizes = 0;
	    while ( fscanf(fp, "%d", &n) != EOF && n != 0 && nsizes < MAXPS )
		pstab[nsizes++] = n;
 ...
 */
    ...
    if ( dwb_mount[m] != NULL ) {
	if ( strcmp(path, dwb_mount[m]->path) == 0 ) {
