...
#define	SF	    35  /* Macro name used to save current fill mode, etc. */
...
extern void cfree(), exit();
extern char *calloc(), *chspace(), *gets1(), *maknew(), *reg(), *strsave();
extern long strtol();
 ...
 *
 * The following items are set by readspec():
 *
 *	nclin - number of format rows
 *
 *	style[col][row] - key letter for this field
 *
 *	font[col][row] - font name or number for this field
 *
 *	csize[col][row] - point size for this field
 *
 *	vsize[icol][row] - vertical spacing value
 *
 *	lefline[col][row] - number of vertical lines immediately to the
 *			    left of this entry
 *
 *	flags[col][row] - flags for this field.  The flag bits are used
 *			  as follows:
 *
 *		ZEROW	001	Zero-width item
 *		HALFUP	002	Half line up
 *		CTOP	004	Put this item at the top of its range.
 *		CDOWN	010	Put this item at the top of its range.
 *
 *	cll[col] - user-specified column width (string)
 *
 *	evenup[col] - next column is equal width (1 = YES, 0 = NO)
 *
 *	sep[col] - separation between this column and the next one
 *
 * The following items are set by gettbl():
 *
 *	instead[nlin] - non-NULL iff this row is a troff request or macro call
 *
 *	fullbot[nlin] - non-zero iff this row is special (full-width horizontal
 *			line or all columns vertically spanned)
 *
 *	stynum[nlin] - index of format row for this data row
 *
 *	table[nlin][icol] - contains pointers to text for this entry
 *
 *	For text blocks, table[nlin][icol].col contains the first
 *	character of the macro name used for the output diversion.
 *
 *	For numeric columns, table[nlin][icol].col contains a pointer to
 *	the text preceding the decimal point.
 *
 *	For alphabetic columns, table[nlin][icol].col contains a pointer
 *	to a null string.
 *
 *	Otherwise, table[nlin][icol].col contains a pointer to the text
 *	for this entry.
 *
 *	For numeric columns, table[nlin][icol].rcol contains a pointer
 *	to the text after the units position, which may be a null string.
 *
 *	For alphabetic columns, table[nlin][icol].rcol contains a pointer
 *	to the text for this entry, which may be a null string.
 *
 *	Otherwise, table[nlin][icol].rcol contains NULL.
 *
 * The following items are set by maktab():
 *
 *	acase[icol] - alphabetic field in this column
 *
 *	doubled[icol] - numeric field in this column
 *
 *	used[icol] - non-zero iff for some value of ilin there is real text
 *		     in table[ilin][icol].col
 *
 *	rused[icol - non-zero iff for some value of ilin there is real text
 *		     in table[ilin][icol].rcol
 *
 *	lused[icol] - non-zero iff for some value of ilin there is real
 *		      text in table[ilin][icol].col and table[ilin][icol].rcol
 *		      is NULL
 *
 *	topat[icol] -
 *
...
int Maxlin = MAXLIN;	/* Maximum number of rows given full treatment */
...
int delim1, delim2;	/* eqn inline-equation delimiters */
int nclin;		/* Number of rows in the format array */
int nlin;		/* Number of rows in the data array */
...
swapin() /* Switch to next input file */
{
	...
	{
		sprintf(errbuf, "Can't open file %s.", sargv[optind]);
		fatal_error(errbuf);
	}
	optind++;
	return(0);
...
do_table() /* Subroutine sequencing for one table */
{
	...
	savefill();	/* Save fill mode, etc. */
	ifdivert();	/* Check for output diversions */
...
do_table1()
{
	...
	multipg();
	maktab();	/* Calculate column widths */
	runout();	/* Draw the table */
	return 0;
}
...
skip_input()	/* Skip remainder of current table */
{
	...
	while (getline(inbuf, sizeof(inbuf)))
	{
...
getspec() /* Allocate data structures and read format lines */
{
	...
	{
		nch = getline(line = inbuf, sizeof(inbuf));
		if (nch == 0)
...
getspec2: /* Come back to here after file inclusion request has been processed */
	...
	for (; nch > 0; nch = getline(line = inbuf, sizeof(inbuf)))
	{
		if (is_table_start(line))
		{
			if (nfd > 0)
	...
	printf(".rm");	/* Clear column-width registers */
	for (i = 0; i < ncol; i++)
...
getchange() /* Process a table-change (.T&) request */
{
	...
	for (;;)
	{
		nch = getline(line = linbuf, sizeof(linbuf));
		if (nch == 0)
...
getchange2: /* Come back to here after file inclusion request has been processed */
	...
	/* Process format lines */
	for (; nch > 0; nch = getline(line = linbuf, sizeof(linbuf)))
	{
		if (is_table_start(line))
		{
			if (nfd > 0)
...
warning(s)	/* Display a warning or error message */
char *s;
{
...
gettbl() /* Read in the first Maxlin lines of data */
{
	...
	for (nlin = 0; (cstore = gets1(inbuf, sizeof(inbuf))) != NULL; nlin++)
	{
		...
		for (icol = 0; icol < ncol; icol++)
		{
			...
			{
				p->col = (char *)gettext(&cstore, nlin, icol);
			}
			else
			{
...
maktab() /* Generate code to perform column width calculations */
{
	...
	printf(".nr %d (\\n(TW*10u+0.5i)/1i\n", TMP);
	printf(".nr %d \\n(%d%%10u\n", S9, TMP); /* Fraction */
	printf(".nr %d \\n(%d/10u\n", TMP, TMP); /* Integer part */
	printf( ".if \\n(TW>\\n(.l "); /* Continued on next line */
	...
	for (ilin = 0; ilin < nlin; ilin++)
	{
		if (k = lspan(ilin, icol))
		{
			...
			printf(".if \\n(%d>0 .nr %d \\n(%d/%d\n",
			    TMP, TMP, TMP, k);
			printf(".if \\n(%d<0 .nr %d 0\n", TMP, TMP);
	/* MR us83-25801 - ik initialized to 0 instead of 1 */
	/* CL 6/88 - terminate at ik<k to pad columns evenly for long span */
			for (ik = 0; ik < k; ik++)
			{
...
runout() /* Generate code to format the table */
{
	...
	setfc(); /* Set field delimiter to F1, pad character to F2 */
	printf(".nr #T 0-1\n");
	deftail();		/* Establish bottom-of-page processing */
	for (i = 0; i < nlin; i++) /* Generate code for first Maxlin rows */
		putline(i, i);
	if (leftover != NULL) /* Process remaining data rows, if any */
		yetmore();
	cleanfc(); /* Turn off field delimiter and pad character mechanism */
	printf(".nr T. 1\n");
	printf(".T# 1\n");
	if (ctrflg)
		printf(".in \\n(#Iu\n");
...
need() /* Generate a troff 'need' request (.ne) to keep the table on one page */
{
/* This uses a simple heuristic to estimate the amount of space required. */
/* It is invoked for boxed tables only. */
	int texlin = 0, horlin = 0, i;

	/* Reserve one vertical line space for each data line, plus two */
	/* points for each full-width or partial-width horizontal line. */
	for (i = 0; i < nlin; i++)
	{
		if (instead[i] != NULL)
			continue;
...
deftail() /* Generate code to define the bottom-of-page macro */
{
	...
	if (boxflg || allflg || dboxflg) /* Bottom of table line */
	{
		if ( ! fullbot[nlin-1])
		{
			...
			/* T. is really an argument to a macro but because of
		   	   eqn we don't dare pass it as an argument and
			   reference by $1 */
		}
	}
	for (c = 0; c < ncol; c++)
	...
	printf(".ec\n"); /* Reenable escape-character mechanism */
}
...
/* nl is line number for finding data (usually identical) */
{
	...
	for (c=0; c<ncol; c++)
	{
		...
		if (form != 's')
		{
			...
			if ((form=='a') ||
			    (form=='n' && TABLE(nl,c).rcol && ! lused[c]))
				cct = reg(c,CMID);
			else
				cct = reg(c,CLEFT);
			printf("\\h'|\\n(%2su'", cct);
...
/* unfolding */
		...
		case '=':
			...
			{
				sprintf(errbuf, "Data ignored on table line %d", i+1);
				warning(errbuf);
			}
...
# define realsplit2 ((ct=='a'||ct=='n') && TABLE(nl,c).rcol)
		...
		/* if lines need to be split here is the place for a backslash */
		if (vct > 7 && c < ncol)
		{
			printf("\n.sp-1\n\\&");
			vct=0;
...
puttext(s, fn, size) /* Generate code to output a text block */
char *s, *size, *fn;
{
	if (point(s))
	{
		...
		printf("%s", s);
		if (*fn) printf("\\f\\n(%2d", S1);
		if (size != NULL) putsize("0");
	}
}

funnies(stl, lin) /* Write out funny diverted things */
/* stl is line number for deciding format */
/* lin is line number for finding data (usually identical) */
{
	int c, pl, ct;
	char *fn, *s;
	...
	for (c=0; c<ncol; c++)
	{
		...
		printf(".in +\\n(%du\n", SIND);
		if (stl>0 && (pl=prev(stl))>=0 && vspen(TABLE(pl,c).col))
		{
			printf(".sp |\\n(^%cu\n", 'a'+c);
...
putfont(fn) /* Generate an inline font-change request */
char *fn;
{
	if (fn != NULL && *fn)
...
putsize(s) /* Generate an inline point-size change request */
char *s;
{
	if (s != NULL && *s)
		printf("\\s%s", s);
...
yetmore() /* Called from runout() when there are more than Maxlin lines */
	  /* of input data */
{
	char *cstore;
...
char *s;
{
	fprintf(stderr, "tbl: File %s, line %d: %s\n", ifile, iline, s);
	errflg = 1;
	if (errterm)
		bailout();
	if (++errcnt1 >= MAX_ERR1)
...
rstofill() /* Restore the environment saved by savefill() */
{
...
endoff() /* Clean up after one table */
{
	...
	for (i = 0; i < texct; i++)
		printf(".rm %c+\n", texstr[i]);
	printf(".ch #f\n");	/* Remove trap for macro #f */
	printf("%s\n", last);	/* Copy .TE to output */
}

ifdivert()
{
	/* Set #d = current vertical place in current diversion */
...
gettext(sp, ilin, icol) /* Read in a text block */
...
{
	if (sizeof(nregs)/sizeof(char *) < qcol*3)
...
/* Returns 1 if the first string is an initial segment of the second string,
   otherwise 0 */
prefix(small, big)
char *small, *big;
{
...
/* lwid determines the kind of line (1=single, 2=double) */
{
	...
	for (ln = 0; ln < lwid; ln++) /* Draws one line on each iteration */
	{
		...
		if (pos != oldpos) printf("\\h'%dp'", pos-oldpos);
