...
Tchar t_setch(int c)
{
	...
	*s = '\0';
	if ((j = chindex(temp)) == -1)
		return chadd(temp) | chbits;	/* add name even if haven't seen it */
						/* "return 0" -> discard unknown char */
	else
		return(j | chbits);
}
...
Tchar t_setabs(void)		/* set absolute char from \N'...' */
{
	...
	if ((n = chindex(temp)) == -1)	/* look for that name */
		n = chadd(temp);
	return n | chbits;
}
...
char *
strdupl(const char *s)	/* make a copy of s */
{
	char *t;
...
setfp(int pos, int f, char *truename, int print)	/* mount font f at position pos[0...nfonts] */
{
	...
	if (truename && strrchr(truename, '/')) {	/* .fp 1 R dir/file: use verbatim */
		...
		sprintf(pathname, "%s/dev%s/%s", fontdir, dwb_devname, truename);
		truename = 0;	/* so doesn't get repeated by ptfpcmd */
	} else					/* vanilla: .fp 5 XX */
		sprintf(pathname, "%s/dev%s/%s", fontdir, dwb_devname, shortname);
