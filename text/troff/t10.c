...
void t_ptinit(void)
{
	...
	if (dwb_devname[0] == 0)
		strcpy(dwb_devname, TDEVNAME);
	hyf = 1;
	lg = 1;

	sprintf(buf, "/dev%s/DESC", dwb_devname);
	strcat(termtab, buf);
...
void t_specnames(void)
{
	...
	for (i = 0; spnames[i].n; i++)
		*spnames[i].n = chindex(spnames[i].v);
}
...
int ptout0(Tchar *pi)
{
	...
	if (k == DRAWFCN) {
		...
		if (esc)
			ptesc();
		if (chnames[k-ALPHABET][0] == '#')	/* DOESN'T WORK: name not nec #dnn */
	...
	if (bd) {
		...
		if (k < ALPHABET) {
			OUT "c%c\n", k PUT;
		} else if (chnames[k-ALPHABET][0] == '#') {	/* DOESN'T WORK: name not nec #dnn */
			OUT "N%s\n", &chnames[k-ALPHABET][1] PUT;
		} else
			OUT "C%s\n", chnames[k - ALPHABET] PUT;
		if (z)
