...
/*IF*/	{ 3, 3, 0, 2, 2, 3, 2, 3, 0, 1, 1 },		/* ILETF */
...
#else /* UNANSI */
char *pad(n)	/* return the padding as a string */
	int n;
#endif /* UNANSI */
{
	static char buf[20];
...
#else /* UNANSI */
void shim(lc, rc)	/* add padding space suitable to left and right classes */
	int lc; int rc;
#endif /* UNANSI */
{
...
#else /* UNANSI */
void roman(c)	/* add char c in "roman" font */
	int c;
#endif /* UNANSI */
{
	nextft = ROM;
...
#else /* UNANSI */
void sadd(s)		/* add string s to cs */
	char *s;
#endif /* UNANSI */
{
	while (*s)
...
#else /* UNANSI */
void cadd(c)		/* add char c to end of cs */
	int c;
#endif /* UNANSI */
{
	char *p;
