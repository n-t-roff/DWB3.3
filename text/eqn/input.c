...
#else /* UNANSI */
void pushsrc(type, ptr)	/* new input source */
	int type; char *ptr;
#endif /* UNANSI */
{
	if (++srcp >= src + MAXSRC)
...
#else /* UNANSI */
void popsrc()	/* restore an old one */
#endif /* UNANSI */
{
	if (srcp <= src)
...
#else /* UNANSI */
void dodef(stp)	/* collect args and switch input to defn */
	tbl *stp;
#endif /* UNANSI */
{
	int i, len;
...
#else /* UNANSI */
void eprint()	/* try to print context around error */
#endif /* UNANSI */
{
	char *p, *q;
