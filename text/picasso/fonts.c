...
initdefaultfont(void)
{
	...
	if (fontcount == 0)	{	/* normal initialization failed */
		dwb_mount[0]->path = "\tinternal\t";
		dwb_mount[0]->name = tostring(troffname[0]);
		dwb_mount[0]->fontname = tostring(troffname[1]);
		dwb_mount[0]->nchars = NDEFWID;
		dwb_mount[0]->state = INMEMORY;
		dwb_mount[0]->mounted++;
		dwb_mount[0]->wp = (Chwid *)malloc(NDEFWID*sizeof(Chwid));
		for (i = 0; i < NDEFWID; i++) {
			dwb_mount[0]->wp[i].wid = defaultwidthtab[i];
			dwb_mount[0]->wp[i].num = i + 32;
		}
		fontcount = 1;
		fcount = mcount = 1;
	}
...
devinit(char *devname)		/* -1  means error; 1 means success */
{
	...
	if (access(filename, 5) != 0) {
		fprintf(stderr, "Can not access troff font directory %s.\n",
								filename);
		if (strcmp(devname, "post") != 0) {
			fprintf(stderr, "Trying %s/devpost.\n", fontdir);
			return devinit("post");
		}
	}
	devfontpath = (char *) malloc(strlen(filename) + 1);
	if (devfontpath != NULL)
		...
		/* fontname, and initializes (mounts) it.    */
{
	...
	for (i = 0; i < fontcount; i++)    /* linear search */
		if (strcmp(dwb_mount[i]->name, fontname) == 0)
			break;
	if (i == fontcount)
...
getstringwidth(unsigned char *s, int fontnum, int ptsize)	/* assumes no font or size changes */
{
	...
	curfont = dwb_mount[fontnum];
	for(; *s; ++s)
		if ((int) *s > 31 && (pos = onfont(*s, fontnum)) != -1)
