...
rightthing(obj *p, int c)	/* called for ... ] or ... } */
{
	...
	/*  If redo_gbox is set we need to recalculate the bounding box
	 *  for this block.
	 */
		if (redo_gbox) {
		    obj	    *o;
		    ...
		    Gbox[0] = Gbox[1] = - (Gbox[2] = Gbox[3] = -32767);
		    for (o = q->o_prev; o != p; o = o->o_prev) {
			if (o->o_type == BLOCKEND)
			    o = o->o_parent;
			if (o->o_type > TEXT)	/* a MOVE could have text? */
			    continue;
			get_bounds(o, bnd, 0);
			track_bounds(bnd[0], bnd[1], bnd[2], bnd[3]);
		    }
		}
		redo_gbox = stack[nstack].p_redo;
...
#if 0
/*	This appears to be unnecessary (and even undesireable!) */
	if (!at && !with)
